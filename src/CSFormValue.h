...
typedef NS_ENUM(NSInteger, CSFormValueType) {
    CSFormValueString,
    CSFormValueBool,
    CSFormValueDate,
    CSFormValueImage,
    CSFormValueMetadata,
    CSFormValueMetadataCollection,
    CSFormValueUnknown
};
 ...
 *  property.
 */
@interface CSFormValue : NSObject
 ...
 */
@property(readonly, nonatomic, assign) CSFormValueType type;
...
/**
 *  Creates and returns a `CSFormValue` object containing the provided value, identified
 *  by the provided type. Only values with supported types will successfully create an
 *  object – the rest will simply return nil.
 *
 *  It's important to note that when creating CSFormValueBool types, either a string or a
 ...
 *
 *  @param aValue A value to be contained by this `CSFormValue` object.
 *  @param type   The type of the value.
 *
 *  @return A new `CSFormValue` object with the provided value. `nil` is returned if the
...
/**
 *  Returns the string value for this `CSFormValue` if it's of that type, or `nil`
 *  otherwise.
 *
 *  @return The string value for this `CSFormValue` if it's of that type or `nil` 
 *          otherwise.
 */
- (NSString *)stringValue;

/**
 *  Returns the bool value for this `CSFormValue` if it's of that type, or NO otherwise.
 *
 *  @return The bool value for this `CSFormValue` if it's of that type or NO otherwise.
 */
- (BOOL)boolValue;

/**
 *  Returns the date value for this `CSFormValue` if it's of that type, or `nil`
 *  otherwise.
 *
 *  @return The date value for this `CSFormValue` if it's of that type or `nil`
 *          otherwise.
 */
- (NSDate *)dateValue;

/**
 *  Returns the image value for this `CSFormValue` if it's of that type, or `nil`
 *  otherwise.
 *
 *  @return The image value for this `CSFormValue` if it's of that type or `nil`
 *          otherwise.
 */
- (UIImage *)imageValue;

/**
 *  Returns the metadata value for this `CSFormValue` if it's of that type, or `nil`
 *  otherwise.
 *
 *  @return The metadata value for this `CSFormValue` if it's of that type or `nil`
 *          otherwise.
 */
- (CSMetadata *)metadataValue;

/**
 *  Returns the metadata collection value for this `CSFormValue` if it's of that type,
 *  or `nil` otherwise.
 *
 *  @return The metadata collection  value for this `CSFormValue` if it's of that 
